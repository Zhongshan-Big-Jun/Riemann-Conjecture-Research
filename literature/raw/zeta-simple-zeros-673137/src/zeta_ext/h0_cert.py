...
def window_monotone_factor_upper(
    spec: KernelSpec, subdivisions: int = 4096
) -> arb:
    """Rigorous upper bound for ``v'(s) / s`` on ``0 < s <= 1/2``.

    Writing ``sinc(z) = sin(z) / z`` removes the zero at the origin:

        v'(s) / s = -sum_j c_j omega_j^2 sinc(omega_j s).

    A nonpositive result proves that the even window is nonincreasing on
    ``[0, 1/2]``.  This supplies the bounded-variation hypothesis used by
    the arbitrary-window analytic interface (and also applies at ``s=0``
    by continuity).
    """
    ...
    for index in range(subdivisions):
        ...
        for coefficient, omega in zip(coeffs, omegas):
            factor -= coefficient * omega * omega * _sinc(omega * cell)
        upper = factor.upper()
