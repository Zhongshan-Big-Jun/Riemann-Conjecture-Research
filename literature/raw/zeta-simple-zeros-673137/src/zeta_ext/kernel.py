...
def sinc_derivatives(z: arb) -> Tuple[arb, arb, arb]:
    ...
    upper = z.abs_upper()
    if arb(upper) == 0:
        return arb(1), arb(0), -arb(1) / 3
    if arb(upper) <= arb(fmpq(3, 4)):
        z2 = z * z
        value = arb(1)
        first = arb(0)
        second = arb(0)
        ...
        up = arb(upper)
        up_pow = up ** (2 * n0)
        # ``arb(mid, radius)`` receives a binary64 radius.  Round the Arb
        # upper bound upward before constructing that ball; nearest rounding
        # here would be needlessly non-rigorous by up to half an ulp.
        value_tail = math.nextafter(
            float(_series_tail_bound(up_pow, n0, 0).abs_upper()),
            math.inf,
        )
        first_tail = math.nextafter(
            float(
                _series_tail_bound(up ** (2 * n0 - 1), n0, 1).abs_upper()
            ),
            math.inf,
        )
        second_tail = math.nextafter(
            float(
                _series_tail_bound(up ** (2 * n0 - 2), n0, 2).abs_upper()
            ),
            math.inf,
        )
        value += arb(0, value_tail)
        first += arb(0, first_tail)
        second += arb(
            0, second_tail
        )
        value = _intersect(value, z.sinc())
        if arb(z.abs_lower()) >= arb(fmpq(1, 20)):
