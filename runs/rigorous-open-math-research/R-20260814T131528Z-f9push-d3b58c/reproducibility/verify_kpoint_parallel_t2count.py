...
def main():
    ...
    ap.add_argument('--emit-boxes',action='store_true',
                    help='collect accepted terminal boxes in memory (for T2 pruning certificate)')
    ap.add_argument('--boxes-out',type=str,default=None,
                    help='write collected accepted boxes as JSON (requires --emit-boxes)')
    args=ap.parse_args()
    ...
    with Pool(workers, initializer=_init,
              initargs=(k, grid, args.precision, target_n, target_d, second_start, args.emit_boxes)) as pool:
    ...
    accepted_count=sum(r['accepted_count'] for r in res)
    if args.emit_boxes:
        all_boxes=[b for r in res for b in (r['accepted_boxes'] or [])]
    else:
        all_boxes=[]
    ...
    print(rep.to_text())
    print(f"T2 accepted terminal boxes: {accepted_count}")
    if args.emit_boxes and args.boxes_out:
        import json
        payload = {
            "format": "t2-pruning-cert-v1",
            "grid": grid,
            "target": f"{target_n}/{target_d}",
            "kernel_table_sha256": table_sha256(table0),
            "accepted_count": accepted_count,
            "boxes": [[list(p) for p in box] for box in all_boxes],
        }
        od = os.path.dirname(os.path.abspath(args.boxes_out)) or '.'
        os.makedirs(od, exist_ok=True)
        fd, tmp = tempfile.mkstemp(dir=od, suffix='.tmp')
        try:
            with os.fdopen(fd, 'w', encoding='utf-8') as fh:
                json.dump(payload, fh, ensure_ascii=False)
            os.replace(tmp, args.boxes_out)
        except BaseException:
            try: os.unlink(tmp)
            except OSError: pass
            raise
        print("written:", args.boxes_out)
    if args.out:
        import tempfile
